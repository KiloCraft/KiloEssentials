...
import org.kilocraft.essentials.user.UserHandler;
...
public final class KiloEssentialsImpl implements KiloEssentials {
    ...
    public KiloEssentialsImpl() {
        if (running) {
            throw new RuntimeException("KiloEssentials is already set up!");
        } else {
            running = true;
        }
        instance = this;
        LOGGER.info("Running KiloEssentials ({})", ModConstants.getVersion());
...
//		return this.commands;
    ...
    public CompletableFuture<Optional<User>> getUserThenAcceptAsync(final ServerCommandSource requester,
        ...
        assert KiloServer.getServer() != null;
