...
public class VanishCommand extends EssentialCommand {
    ...
    private int toggle(final CommandContext<ServerCommandSource> ctx) throws CommandSyntaxException {
        final OnlineServerUser user = (OnlineServerUser) KiloServer.getServer().getOnlineUser(ctx.getSource().getPlayer());

        user.getPreferences().set(preference, !user.getPreference(preference));


        //TODO: Cancel join event
        List<OnlineUser> online = KiloEssentials.getServer().getUserManager().getOnlineUsersAsList();
        if (user.getPreference(preference)) {
            ...
            //KiloEssentials.getServer().getPlayerManager().sendToAll(new PlayerListS2CPacket(PlayerListS2CPacket.Action.REMOVE_PLAYER, user.asPlayer()));
            ...
            //KiloEssentials.getServer().getPlayerManager().sendToAll(new PlayerListS2CPacket(PlayerListS2CPacket.Action.ADD_PLAYER, user.asPlayer()));
