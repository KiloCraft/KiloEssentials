            ...
            if (set) {
                if (level > leastPermLevelReq) source.sendError(KiloCommands.getPermissionError("Operator permission level " + (leastPermLevelReq + 1)));
                    ...
                    if (!playerManager.isOperator(gameProfile)) {
                        ...
                        if (CommandHelper.isOnline(p)) p.addMessage(LangText.getFormatter(true, "command.operator.announce", source.getName(), level), false);
                ...
                removeOperator(gameProfile);
                if (CommandHelper.isOnline(p)) p.addMessage(LangText.get(true, "command.operator.announce.removed"), false);
